...
export default defineConfig(({ command, mode, ssrBuild }) => {
	return {
				...
				configureServer: {
					...
					handler: (server) => {
						if (isHost) {
							...
							logger(`_DEV_HOST_ADDRESS`, `http://${templateConfig.server.hostname}:${templateConfig.server.port}`)
							logger(`_DEV_DONE`)
						}
					}
				}
		...
		build: {
			...
			assetsInlineLimit: 0,
			rollupOptions: {
				input: globSync('./src/*.html', { ignore: [`./src/${templateConfig.devcomponents.filename}`] }),
				plugins: [
